...
class IVS_Accessory_Framework(App):
	...
	def load_kv_files(self):
		...
		Builder.load_file('Keypadv3/keypad.kv')
	def build(self):
			...
			if section == "valt":
				...
				if key == "roomname":
					...
					if type(valtrooms).__name__ == 'list':
						...
						if key == "server":
							print("test")
							config.set('valt', 'roomname', None)
							config.set('valt', 'room', None)
							self.write_config()
							try:
	...
	def connsuccess(self):
		...
		if roomname == 0:
			self.screenmgmt.get_screen(self.homescreen).ids['feedback'].text = self.valt.errormsg
			self.screenmgmt.get_screen(self.homescreen).ids['feedback'].color = (1, 0, 0, 1)
		else:
		...
		try:
			...
			pass
		self.event_checkroomstatus = Clock.schedule_interval(self.checkroomstatus,5)
	def connfailure(self):
		...
		self.screenmgmt.get_screen(self.homescreen).ids['feedback'].text = "Unable to connect to VALT"
		self.screenmgmt.get_screen(self.homescreen).ids['feedback'].color=(1,0,0,1)
	...
	def startrecording(self):
		#if self.valt.getroomstatus(self.valtconfig["room"]) == 1 or self.valt.getroomstatus(self.valtconfig["room"]) == 5:
		if self.valt.startrecording(self.config.get("valt","room"),self.config.get("valt","recname")) != 0:
			self.updrecon()
		else:
			self.screenmgmt.get_screen(self.homescreen).ids['feedback'].text = self.valt.errormsg
	def stoprecording(self):
		#if self.valt.getroomstatus(self.valtconfig["room"]) == 2 or self.valt.getroomstatus(self.valtconfig["room"]) == 3:
		if self.valt.stoprecording(self.config.get("valt","room")) != 0:
			self.updrecoff()
		else:
			self.screenmgmt.get_screen(self.homescreen).ids['feedback'].text = self.valt.errormsg
	def pauserecording(self):
		#if self.valt.getroomstatus(self.valtconfig["room"]) == 2:
		if self.valt.pauserecording(self.config.get("valt","room")) != 0:
			self.updpause()
		else:
			self.screenmgmt.get_screen(self.homescreen).ids['feedback'].text = self.valt.errormsg
	def resumerecording(self):
		#if self.valt.getroomstatus(self.valtconfig["room"]) == 3:
	...
	def updlock(self):
		self.screenmgmt.get_screen(self.homescreen).ids.recording_time.text = "Room is Locked"
		self.screenmgmt.get_screen(self.homescreen).ids.recording_time.color = (0,0,0,1)
		self.screenmgmt.get_screen(self.homescreen).ids.recording_layout.clear_widgets()
	def updprepared(self):
		self.screenmgmt.get_screen(self.homescreen).ids.recording_time.text = "Room is Prepared"
		self.screenmgmt.get_screen(self.homescreen).ids.recording_time.color = (1,.5,0,1)
	...
	def addrecordingbutton(self):
		self.screenmgmt.get_screen(self.homescreen).ids.recording_layout.clear_widgets()
		if self.config.get("application", "orientation") == "Landscape":
			btn = ImageButton(id='start_button', size_hint=(1, 1), source=self.imagepath + 'Start_Rec.png', upimage=self.imagepath + 'Start_Rec.png', downimage=self.imagepath + 'Start_Rec_down.png', always_release=True, on_release=lambda x: self.startrecording())
			self.screenmgmt.get_screen(self.homescreen).ids.recording_layout.add_widget(btn)
		else:
			btn = ImageButton(id='start_button', size_hint=(1, 1), source=self.imagepath + 'record_icon.png', upimage=self.imagepath + 'record_icon.png', downimage=self.imagepath + 'record_icon.png', always_release=True, on_release=lambda x: self.startrecording())
			self.screenmgmt.get_screen(self.homescreen).ids.recording_layout.add_widget(btn)
			label = Label(size_hint=(1,1))
			self.screenmgmt.get_screen(self.homescreen).ids.recording_layout.add_widget(label)
			# btn = ImageButton(id='start_button', size_hint=(1, 1), source=self.imagepath + 'rec_small.png', upimage=self.imagepath + 'rec_small.png', downimage=self.imagepath + 'rec_small.png', always_release=True, on_release=lambda x: self.startrecording())
		# self.screenmgmt.get_screen(self.homescreen).ids.recording_layout.add_widget(btn)
	...
	def addunlockbutton(self):
		self.screenmgmt.get_screen(self.homescreen).ids.privacy_layout.clear_widgets()
		if self.config.get("application", "orientation") == "Landscape":
			btn = ImageButton(id="privacy_button", size_hint=(1, 1), source=self.imagepath + 'Room_locked.png', upimage=self.imagepath + 'Room_locked.png', downimage=self.imagepath + 'Room_locked_down.png', always_release=True, on_release=lambda x: self.disableprivacy())
		else:
			btn = ImageButton(id="privacy_button", size_hint=(1, 1), source=self.imagepath + 'locked_icon.png', upimage=self.imagepath + 'locked_icon.png', downimage=self.imagepath + 'locked_icon.png', always_release=True, on_release=lambda x: self.disableprivacy())
		self.screenmgmt.get_screen(self.homescreen).ids.privacy_layout.add_widget(btn)
	def addlockbutton(self):
		self.screenmgmt.get_screen(self.homescreen).ids.privacy_layout.clear_widgets()
		if self.config.get("application", "orientation") == "Landscape":
			btn = ImageButton(id="privacy_button", size_hint=(1, 1), source=self.imagepath + 'Room_unlocked.png', upimage=self.imagepath + 'Room_unlocked.png', downimage=self.imagepath + 'Room_unlocked_down.png', always_release=True, on_release=lambda x: self.enableprivacy())
		else:
			btn = ImageButton(id="privacy_button", size_hint=(1, 1), source=self.imagepath + 'unlocked_icon.png', upimage=self.imagepath + 'unlocked_icon.png',	downimage=self.imagepath + 'unlocked_icon.png', always_release=True, on_release=lambda x: self.enableprivacy())
		self.screenmgmt.get_screen(self.homescreen).ids.privacy_layout.add_widget(btn)
	def addpausestopbuttons(self):
		self.screenmgmt.get_screen(self.homescreen).ids.recording_layout.clear_widgets()
		if self.config.get("application", "orientation") == "Landscape":
			btn = ImageButton(id="pause_button", size_hint=(1, 1), source=self.imagepath + 'small_pause.png', upimage=self.imagepath + 'small_pause.png', downimage=self.imagepath + 'small_pause_down.png', always_release=True, on_release=lambda x: self.pauserecording())
		else:
			btn = ImageButton(id="pause_button", size_hint=(1, 1), source=self.imagepath + 'pause_icon.png', upimage=self.imagepath + 'pause_icon.png', downimage=self.imagepath + 'pause_icon.png', always_release=True, on_release=lambda x: self.pauserecording())
		self.screenmgmt.get_screen(self.homescreen).ids.recording_layout.add_widget(btn)
		if self.config.get("application", "orientation") == "Landscape":
			btn = ImageButton(id="stop_button", size_hint=(1, 1), source=self.imagepath + 'small_stop.png', upimage=self.imagepath + 'small_stop.png', downimage=self.imagepath + 'small_stop_down.png', always_release=True, on_release=lambda x: self.stoprecording())
		else:
			btn = ImageButton(id="stop_button", size_hint=(1, 1), source=self.imagepath + 'stop_icon.png', upimage=self.imagepath + 'stop_icon.png', downimage=self.imagepath + 'stop_icon.png', always_release=True, on_release=lambda x: self.stoprecording())
		self.screenmgmt.get_screen(self.homescreen).ids.recording_layout.add_widget(btn)
		self.screenmgmt.get_screen(self.homescreen).ids['recording_layout'].padding = 2
	...
	def addresumebutton(self):
		self.screenmgmt.get_screen(self.homescreen).ids.recording_layout.clear_widgets()
		if self.config.get("application", "orientation") == "Landscape":
			btn = ImageButton(size_hint=(1,1),source=self.imagepath + 'resume.png',upimage=self.imagepath + 'resume.png',downimage=self.imagepath + 'resume_down.png',always_release=True,on_release= lambda x: self.resumerecording())
		else:
			btn = ImageButton(size_hint=(1, 1), source=self.imagepath + 'resume_icon.png', upimage=self.imagepath + 'resume_icon.png', downimage=self.imagepath + 'resume_icon.png', always_release=True, on_release=lambda x: self.resumerecording())
		self.screenmgmt.get_screen(self.homescreen).ids.recording_layout.add_widget(btn)
	...
	def getnetworkinfo(self):
		...
		for nic in netifaces.interfaces():
			#print(nic)
			if nic != 'lo':
				lbl = HeaderLabel(text=nic,size_hint_y=None,height=30)
				...
				lbl = StandardTextLabel(text="MAC Address:", width=lblwidth, size_hint_x = None)
				textbox = DisabledTextInput(text=netifaces.ifaddresses(nic)[netifaces.AF_LINK][0]['addr'])
				...
				for conf in netifaces.ifaddresses(nic):
					for x in netifaces.ifaddresses(nic)[conf]:
						...
						if ivs.is_ipv4(x['addr']):
							lbl = StandardTextLabel(text="IP:", width=lblwidth, size_hint_x = None)
							textbox = DisabledTextInput(text=x['addr'])
							self.screenmgmt.get_screen('About_Screen').ids['network_info_layout'].ids[nic + '_layout'].add_widget(lbl)
							self.screenmgmt.get_screen('About_Screen').ids['network_info_layout'].ids[nic + '_layout'].add_widget(textbox)
							if "netmask" in x:
								lbl = StandardTextLabel(text="Subnet Mask:", width=lblwidth, size_hint_x = None)
								textbox = DisabledTextInput(text=x['netmask'])
								self.screenmgmt.get_screen('About_Screen').ids['network_info_layout'].ids[nic + '_layout'].add_widget(lbl)
								self.screenmgmt.get_screen('About_Screen').ids['network_info_layout'].ids[nic + '_layout'].add_widget(textbox)
						#except:
						#		pass
	...
	def build_application_settings(self):
		...
		self.build_settings_json(self.applicationjson, settype="title", title="Settings")
	...
	def build_settings_defaults(self):
		self.defaultsjson = {}
		self.defaultsjson['application'] = {"settingspassword":"admin51","mode":"Camera Control","webinterface": "1","webpassword": None,"recbutton":"1","privbutton":"1","orientation":"Landscape"}
	...
	def load_app_window(self):
		if self.config.get("application","mode") == "Camera Control":
			lw=CameraControl(self.valt,self.config.get("valt","room"))
